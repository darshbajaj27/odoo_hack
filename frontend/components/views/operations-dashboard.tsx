  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8 bg-white dark:bg-slate-950 text-gray-900 dark:text-white min-h-screen">
      <div>
        ...
        <p className="text-slate-600 dark:text-gray-400 mt-2">Manage incoming receipts, deliveries, and manufacturing orders</p>
      </div>
        ...
        {operations.map((op) => {
          ...
          return (
            ...
            >
              ...
              <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-4">{op.title}</h3>

              <div className="space-y-3">
                <button
                  onClick={() => onNavigate("operations-list")}
                  className="w-full h-11 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white font-semibold rounded-lg transition-colors"
                >
                  {op.count} To Process
                </button>
                {op.late > 0 && <p className="text-sm text-red-600 dark:text-red-400 font-medium">{op.late} Late</p>}
              </div>
            </div>
          )
        })}
